...
class Snake {
public:
    Snake(const int fieldRows, const int fieldCols, const Point& startPos);

    ~Snake();

    StatusCode move(const Direction dir, const std::vector<Point>& obstacles, std::vector<Point>& powerUps);
