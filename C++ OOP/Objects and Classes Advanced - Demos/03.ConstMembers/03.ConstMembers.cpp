...
public:
    ...
    Company(std::string id, long long capitalDollars) 
        : capitalDollars(capitalDollars), id(id) {}
...
int main() {
    ...
    // c.id = "thiswontcompile"; 
    // compilation error - attempting to modfy a const field

    constRef.print();
    c.addCapital(999999);
    constRef.print();
    // constRef.addCapital(999999);
    // compilation error - calling non-const method on const reference
