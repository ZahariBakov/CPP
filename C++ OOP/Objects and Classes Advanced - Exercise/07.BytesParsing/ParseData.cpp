...
CommandType getCommandType(const char commandChar) {
	...
	case 'l':
		...
		break;
	default:
		std::cerr << "Warning, unknown commandChar reveived: "
...
long long parseSingleNumber(CommandType commandType, char* buffer) {
...
ErrorCode parseData(const std::string& commands,
	...
	std::vector<long long>& ourParsedNumbers) {
	size_t parsedSize = 0;
	...
	for (size_t i = 0; i < commands.size(); ++i) {
		...
		//parse the data
		ourParsedNumbers.push_back(parseSingleNumber(commandType, rawDataPtr));
