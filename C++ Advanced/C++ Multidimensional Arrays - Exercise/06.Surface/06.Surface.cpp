...
// A difference between the two solutions is the use of a stack or queue.
...
void BFS(int currX, int currY,
    std::vector<std::vector<char>>& matrix,
    std::queue<std::pair<int, int>>& futureCoords,
    int& currLakeSize) {
    const bool isInside = isInBoundary(matrix, currX, currY);
    if (!isInside) {
        return;
...
std::vector<int> executeCommands(const std::vector<std::vector<char>>& matrix,
    ...
    for (const auto& pair : commands) {
        ...
        while (!futureCoords.empty()) {
            currCoord = futureCoords.front();
            futureCoords.pop();

            // process
            BFS(currCoord.first, currCoord.second, matrixCopy, futureCoords, currLakeSize);
        }
