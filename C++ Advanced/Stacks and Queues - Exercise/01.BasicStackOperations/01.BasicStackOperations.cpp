...
void removeElement(const int numbersToRemove, std::stack<int>& stackNumbers) {
...
std::string findSolution(const int targetNumber, std::stack<int>& stackNumbers) {
    std::string result;
    ...
    while (!stackNumbers.empty()) {
        currentNumber = stackNumbers.top();
